...
useEffect(() => {
  const fetchSuppliersAndOffers = async () => {
    try {
      ...
      // ✅ Build price map (material_name → price)
      const priceMap = {};
      offersRes.data.forEach((offer) => {
        priceMap[offer.material_name] = offer.price;
      });
      setItemPrices(priceMap); // ✅ This fixes your undefined error
...
{isReceiptModalOpen && selectedReceipt && (
  <div className="custom-modal-backdrop">
    <div className="custom-modal" style={{ width: "450px" }}>
      ...
      {uploadedFileUrl ? (
        // Show button to view uploaded image
        <button
        ...
        // Show file input & upload button if no image uploaded yet
        <>
          <input
            type="file"
            accept="image/*"
          ...
          <button
            ...
            onClick={async () => {
              if (!receiptFile) return alert("Please select an image first.");
              ...
              try {
                ...
                setUploadedFileUrl(response.data.file_url); // Save uploaded file URL
              } catch (err) {
