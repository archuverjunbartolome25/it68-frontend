...
// Inside handleSaveDisposal()
for (const item of items) {
  const piecesPerCaseMap = {
    "350ml": 24,
    "500ml": 24,
    "1L": 12,
    "6L": 1,
  ...
  const piecesPerCase = piecesPerCaseMap[item.item] || 1;
  const quantityInPcs = item.quantity; // user input in pieces

  // 1️⃣ Save disposal record
  await axios.post("http://localhost:8000/api/disposals", {
    ...
    reason: newDisposal.reason || "Not specified",
  });

  // 2️⃣ Deduct from inventory (pieces only)
  await axios.post("http://localhost:8000/api/inventories/deduct", {
    type: item.item_type,
    item: item.item,
    quantity: quantityInPcs,
    unit: "pcs",
    ...
    setNewDisposal({
      ...
      selectedType: "Finished Goods",
      productQuantities: { "350ml": 0, "500ml": 0, "1L": 0, "6L": 0 },
      rawMaterialQuantities: { "350ml": 0, "500ml": 0, "1L": 0, "6L": 0, "Cap": 0, "6L Cap": 0, "Label": 0 },
    });
  } catch (err) {
      ...
      <div className="main-content">
        ...
        <div className="d-flex justify-content-between align-items-center mb-3 mt-3 flex-wrap">
          ...
          <div className="ms-auto d-flex gap-2">
            ...
            >
              + Add {newDisposal.selectedType === "Raw Materials" ? "Raw Materials" : "Finished Goods"}
            </button>
            <button
          ...
          <div className="custom-modal-backdrop">
            <div className="custom-modal" style={{width:"450px"}}>
              <div className="modal-header">
                <h5><strong>Add For Disposal</strong></h5>
                      ...
                      <tr key={row.id}>
                        <td>
                          <select
                            ...
                            onChange={(e) => {
                              const updated = newDisposal.items.map((item) =>
                                item.id === row.id ? { ...item, item: e.target.value } : item
                              );
                          ...
                          >
                            ...
                            {newDisposal.selectedType === "Finished Goods" &&
                              ["350ml", "500ml", "1L", "6L"].map(it => (
                                <option key={it} value={it}>{itemDisplayNames[it] || it}</option>
                              ))}
                            {newDisposal.selectedType === "Raw Materials" &&
                              ["350ml_raw", "500ml_raw", "1L_raw", "6L_raw", "Cap", "6L Cap", "Label", "Stretchfilm", "Shrinkfilm"].map(it => (
                                <option key={it} value={it}>{itemDisplayNames[it] || itemDisplayNames[it] || it}</option>
                              ))}
                          </select>
                        </td>
                        <td>
  ...
  <table className="table table-striped table-sm">
    ...
    <tbody>
      <tr>
        ...
        <td>{itemDisplayNames[selectedDisposal.item] || selectedDisposal.item}</td>
