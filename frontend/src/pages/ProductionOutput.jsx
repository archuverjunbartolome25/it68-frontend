...
const handleRowClick = async (record) => {
  try {
    ...
    console.log("BOM data from API:", res.data); // ✅ Correct variable

    setSelectedProduction({ ...record, bom: res.data });
...
const handleAddProduction = async (item, batchNum) => {
  try {
    const { product, unit, quantity } = item;
    ...
    const perCase = conversionRates[product] || 1;
    ...
    if (unit === "pieces") {
      quantity_pcs = qty;
      quantity_cases = Math.floor(qty / perCase);
    } else if (unit === "cases") {
      quantity_cases = qty;
      quantity_pcs = qty * perCase;
    }

const selectedSuppliers = {};
item.rawMaterials?.forEach(rm => {
  selectedSuppliers[rm.name] = rm.selectedSupplier || rm.suppliers[0];
});
...
const payload = [
  {
    ...
    rawMaterials: item.rawMaterials?.map(rm => ({
      name: rm.name,
      supplier: rm.selectedSupplier || rm.suppliers[0],
        ...
        <div key={index} className="mb-3 p-2 border rounded">
          <div className="mb-2">
              ...
              onChange={async (e) => {
                ...
                try {
                  const res = await axios.get(
                    `http://localhost:8000/api/raw-materials/by-product/${e.target.value}`
                  );
                  ...
                  setShowSupplierModal(true); // 👈 open the supplier modal
                } catch {
            ...
            >
              ...
              {["350ml","500ml","1L","6L"].map(p => <option key={p} value={p}>{p}</option>)}
            </select>
          </div>
          ...
          {item.rawMaterials?.map((rm, rmIndex) => rm.suppliers?.length > 0 && (
  <div key={rmIndex} className="mb-2">
    <label className="form-label">{rm.material} Supplier:</label>
    <select
      className="form-select"
      value={rm.selectedSupplier || ""}
      onChange={(e) => {
        const updated = [...addedItems];
        updated[index].rawMaterials[rmIndex].selectedSupplier = e.target.value;
        setAddedItems(updated);
      }}
    >
    ...
    <div className="custom-modal" style={{ width: "450px", maxHeight: "80vh", overflowY: "auto" }}>
      ...
      <div className="text-end mt-3">
        ...
        <button
          ...
          onClick={() => {
            const updated = [...addedItems];
            updated[selectedItemForSuppliers.index].rawMaterials = selectedItemForSuppliers.rawMaterials;
            setAddedItems(updated);
    ...
    >
      ...
      {summaryData.map((item, index) => (
        <div key={index} className="mb-3 p-3 border rounded">
          <h6 className="mb-2"><strong>Product:</strong> {item.product}</h6>
          <p className="mb-1"><strong>Unit:</strong> {item.unit}</p>
          <p className="mb-2"><strong>Quantity:</strong> {item.quantity}</p>

          {/* Raw Materials Used */}
          {item.rawMaterials?.length > 0 && (
            <>
              <p className="mt-3 mb-1"><strong>Raw Materials Used</strong></p>
                ...
                <tbody>
                  {item.rawMaterials.map((rm, i) => (
                    <tr key={i}>
                      ...
                      <td>
                        {rm.selectedSupplier
                          ? rm.selectedSupplier.name || rm.selectedSupplier
                          : (
        ...
        <button
          ...
          onClick={async () => {
            try {
              ...
              await Promise.all(
                summaryData.map((item) => handleAddProduction(item, newBatch))
              );
              fetchOrders();
              showMessage("✅ All production outputs saved successfully!");
              setAddedItems([]);
              setShowAddModal(false);
              setShowSummaryModal(false);
            } catch (err) {
              console.error(err);
              showMessage("❌ Failed to save production outputs.");
            }
          }}
        >
          Confirm & Save
...
{showDetailsModal && selectedProduction && (
  <div className="custom-modal-backdrop">
    <div className="custom-modal w-50">
      <div className="modal-header d-flex justify-content-between align-items-center">
        <h5 className="modal-title"><strong>Bill of Materials</strong></h5>
        <button
          type="button"
          className="btn-close"
      ...
      <div className="modal-body">
        <p><strong>Batch #:{selectedProduction.batch_number}</strong></p>
        <p>
          <strong>Production Date/Time:</strong>{" "}
          {new Date(selectedProduction.production_date + " UTC").toLocaleString(
            "en-PH",
            { timeZone: "Asia/Manila" }
          )}
        </p>

{selectedProduction?.bom?.map((product, index) => (
  <div key={index} className="mb-4">
    <h5 className="fw-bold mb-2">{product.product_name}</h5>

{product.materials_grouped?.map((group, gIdx) => (
  <div key={gIdx} className="mb-3">
    <h6 className="text-primary fw-semibold">Supplier: {group.supplier}</h6>
      ...
      <tbody>
        {group.materials.map((mat, i) => (
          <tr key={i}>
            ...
            <td>
              ₱{(mat.unit_price ?? 0).toLocaleString(undefined, {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
              })}
            </td>
            <td>
              ₱{(mat.total ?? 0).toLocaleString(undefined, {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
              })}
            </td>
        ...
        <tr className="table-secondary fw-semibold">
          ...
          <td>
            ₱{group.materials
              .reduce((sum, mat) => sum + (mat.total ?? 0), 0)
